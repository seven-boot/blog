...
![not remove or add in foreach](assets/not remove or add in foreach.webp)

但是手册中并没有给出具体的原因，下面来分析以下该规定背后的思考：
...
其实，经过debug后，我们可以发现，如果remove代码没有被执行过，iterator.next这一行是一直没报错的。抛异常的时机也正是remove执行之后的那一次next方法的调用。
...
至此，我们介绍清楚了不能在 foreach 循环体中直接对集合进行add/remove操作的原因。
...
java.util.concurrent包下的容器都是 fail-safe，可以在多线程下并发使用，并发修改。
